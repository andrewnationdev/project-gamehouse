...
export default function CartDrawerComponent(props: ICartDrawerProps) {
  return <div
    ...
    <div
        ...
        {props.cart.map((item, i) => (
          <div key={i} className="flex justify-between items-center py-2 border-b border-gray-700">
            ...
            <div className="flex items-center gap-4">
              <span className="font-bold text-green-400 text-xs w-16">R$ {item.price.toFixed(2)}</span>
      ...
      {props.cart.length > 0 && 
      <button className="w-full mt-6 cursor-pointer bg-green-600 py-3 rounded font-bold hover:bg-green-500 transition shadow-lg">
        Finalizar (R$ {props.cart.reduce((acc, item) => acc + item.price, 0).toFixed(2)})
      </button>}
    </div>
  </div>
}